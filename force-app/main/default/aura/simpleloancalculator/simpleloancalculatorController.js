({
    ...
    doInit:function(componet,event,helper){
        ...
        if(str.indexOf('Lead') != -1){
            ...
            console.log('Lead found ==========================='+componet.get("v.sobjectName"));
        }
        else if(str.indexOf('Opportunity') != -1){ 
            componet.set("v.sobjectName","Opportunity");
            console.log('Opportunity found ==========================='+componet.get("v.sobjectName"));
        }
    },
            ...
            case "AffordabilityCalculator":
                ...
                switch(acMethodCal){
                    case "3":
                        ...
                        ExistingMonthlyPayments=component.find("existingMthlyPayment").get("v.value");
                        break;
                    case "2":
                        DesiredMonthlyPayments=component.find("desiredMonthly").get("v.value");
                        break;
                }
                
                var newLoan_Calculator = {'sObject':'Loan_Calculator__c',
                                          'Lead_and_Referral__c':leadid,
                                          'Opportunity__c':oppid,
                                          'Interest_Rate__c':component.find("InterestRate").get("v.value"),
                                          'Maximum_Loan_Amount__c':component.find("maximumLoanamt").get("v.value"),
                                          ...
                                          'Method_Of_Calculation__c':((component.find("methodCalucaltion").get("v.value")=='2')? 'Desired Monthly Payment':'Affordability')                      
                                         };
                console.log('newLoan Affordability Calculator==>'+newLoan_Calculator);
                var dataForSave = JSON.stringify(newLoan_Calculator);
                helper.saveCalculation(component,dataForSave,SelectedRadioGrpValue,event);
                break;
            case "InstallmentPayment":
                var newLoan_Calculator = {'sObject':'Loan_Calculator__c',
                                          'Lead_and_Referral__c':leadid,
                                          'Opportunity__c':oppid,
                                          'Interest_Rate__c':component.find("InterestRatePayment").get("v.value"),
                                          'Years__c':component.find("loanTermYrPayment").get("v.value"),
                                          'Months__c':component.find("loanTermMthPayment").get("v.value"),
                                          'Total_Monthly__c': component.find("maximumLoanamtPayment").get("v.value"),
                                          'Other_Financing__c':component.find("otherfinancingPayment").get("v.value"),
                                          'Maximum_Loan_Amount__c' :component.find("loanAmtPayment").get("v.value")
                                         };
                console.log('newLoan Installment Payment Calculator==>'+newLoan_Calculator);
                var dataForSave = JSON.stringify(newLoan_Calculator);
                helper.saveCalculation(component,dataForSave,SelectedRadioGrpValue,event);
                break;
            case "RevolvingCreditLimitUnsecured":
                console.log('Revolving Credit Limit Unsecured Save called=========================');
                var newLoan_Calculator = {'sObject':'Loan_Calculator__c',
                                          'Lead_and_Referral__c':leadid,
                                          'Opportunity__c':oppid,
                                          'Product_Type__c':((component.find("productType").get("v.value")=='1')? 'Credit Card':'Line of credit'),
                                          'Reqeusted_Limit__c':component.find("requestedlimitRCL").get("v.value"),
                                          'Interest_Rate__c':component.find("interestrateRCL").get("v.value"),
                                          'Monthly_Gross_Income__c':component.find("grossmonthlyincomeRCL").get("v.value"),
                                          'Existing_Monthly_Credit_Payment__c': component.find("existingmonthlycreditpayment").get("v.value"),
                                          'Total_Monthly__c':component.find("totalmonthlyPaymentUnsecure").get("v.value"),
                                         };
                console.log('Revolving Credit Limit Unsecured Calculator==>'+newLoan_Calculator);
                var dataForSave = JSON.stringify(newLoan_Calculator);
                helper.saveCalculation(component,dataForSave,SelectedRadioGrpValue,event);
                break;
            case "RevolvingCreditLimitSecured":
                console.log('Revolving Credit Limit Secured Save called=========================');
                var CollateralType='';
                ...
                var newLoan_Calculator = {'sObject':'Loan_Calculator__c',
                                          'Lead_and_Referral__c':leadid,
                                          'Opportunity__c':oppid,
                                          'Product_Type__c':((component.find("prodTypeSecured").get("v.value")=='1')? 'Credit Card':'Line of credit'),
                                          'Collateral_Type__c' : CollateralType,
                                          'Reqeusted_Limit__c':component.find("requestedlimitRCLsecure").get("v.value"),
                                          'Interest_Rate__c':component.find("interestRateprocal").get("v.value"),
                                          'Value_of_Security__c':component.find("valueofSecurity").get("v.value"),
                                          'Proposed_Starting_Limit__c': component.find("proposeStartingLimit").get("v.value")
                                         };
                console.log('Revolving Credit Limit Secured Calculator==>'+newLoan_Calculator);
                var dataForSave = JSON.stringify(newLoan_Calculator);
                helper.saveCalculation(component,dataForSave,SelectedRadioGrpValue,event);
                break;
    ...
    showHideCalc : function(component, event, helper) {
        ...
        $A.util.addClass(component.find("RevolvingCreditLimitUnsecured"),"slds-hide");
        $A.util.addClass(component.find("RevolvingCreditLimitSecured"),"slds-hide");
        ...
        switch(selCalc){
            case "AffordabilityCalculator":
                ...
                */
                ...
                var acexistingMthlyPayment = cmp.find("existingMthlyPayment").get("v.value");
                var acMethod = cmp.find("methodCalucaltion").get("v.value");
                ...
                else if( cmp.find("existingMthlyPayment").get("v.value")>0 && cmp.find("GrossIncome").get("v.value")>0 &&  acMethodCal=='3' && cmp.find("maximumLoanamt").get("v.value")>0){
                    $A.util.removeClass(cmp.find("affordablitySave"),"slds-hide");
                }
                    else{
                        $A.util.addClass(cmp.find("affordablitySave"),"slds-hide");
            ...
            case "InstallmentPayment":
                ...
                var ipTerm = ((cmp.find("loanTermYrPayment").get("v.value")*12)+parseFloat(cmp.find("loanTermMthPayment").get("v.value")));
                var iploanAmtPayment = cmp.find("loanAmtPayment").get("v.value");
                var ipotherfinancingPayment = cmp.find("otherfinancingPayment").get("v.value");
                console.log(ipTerm);
                var totalLoanamt = parseFloat(iploanAmtPayment) + parseFloat(ipotherfinancingPayment);
            ...
            case "RevolvingCreditLimitUnsecured":
                ...
                var grossmonthlyincomeRCL = cmp.find("grossmonthlyincomeRCL").get("v.value");
                var existingmonthlycreditpayment = cmp.find("existingmonthlycreditpayment").get("v.value");
                var grossmonthly = cmp.find("grossmonthlyincomeRCL").get("v.value");
                var totalmonthlyPaymentUnsecure = cmp.find("totalmonthlyPaymentUnsecure").get("v.value");
                ...
                switch(productType){ 
                    case "1":
                        ...
                        console.log("xtotalmonthlyPaymentUnsecure: x:"+proposedStartingLimit);    
                        break;  
                    case "2":
                        ...
                        var proposedStartingLimit = Math.min(proposedLimit_beforecredit*startingCreditLimit,requestedlimitRCL);
                        if(requestedlimitRCL>0)
                            proposedStartingLimit = Math.min(proposedLimit_beforecredit*startingCreditLimit,requestedlimitRCL);
                        else
                            proposedStartingLimit = (proposedLimit_beforecredit*startingCreditLimit);
                        console.log("proposedStartingLimit"+proposedStartingLimit);
                        cmp.find("totalmonthlyPaymentUnsecure").set("v.value",helper.RoundTo(proposedStartingLimit,10000)); 
                        break;
            ...
            case "RevolvingCreditLimitSecured":
                ...
                var requestedlimitRCL = cmp.find("requestedlimitRCLsecure").get("v.value");
                ...
                switch(productType){ 
                    case "1":
                        ...
                        console.log("MaximumlimitCalc_accountbal "+MaximumlimitCalc_accountbal);  
                        var monthlyPrincpalPayment = (2.5/100);
                        console.log("monthlyPrincpalPayment "+monthlyPrincpalPayment);
                        var MinpaymentOfOutstandingBalance = CardInterest+monthlyPrincpalPayment;
                    ...
                    case "2":   
                        ...
                        console.log("monthlyPrincpalPayment "+monthlyPrincpalPayment);
                        var MinpaymentOfOutstandingBalance = (parseFloat(CardInterest)+parseFloat(monthlyPrincpalPayment));
                        console.log("MinpaymentOfOutstandingBalance "+MinpaymentOfOutstandingBalance);
