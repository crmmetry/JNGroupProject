({
  ...
  saveCalculation: function (component, calculatedData, radioGrpValue, event) {
    console.log(
      "SimpleLoanCalculationHelper:saveCalculation Called " +
        calculatedData +
        ".......radioGrpValue==>" +
        radioGrpValue
    );
  ...
  validateAffordabilityCalculator: function (cmp, event, acMethod) {
    //JN1-2459
    var flag = false;
  ...
  validateInstallmentPaymentCalculator: function (cmp, event) {
    //JN1-2459
    var flag = false;
    var InterestRate = cmp.find("InterestRatePayment").get("v.value");
    console.log(
      "InstallmentPayment validation InterestRate: =====>" + InterestRate
    );
    if (InterestRate == "0") {
