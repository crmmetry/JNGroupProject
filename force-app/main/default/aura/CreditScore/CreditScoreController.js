({
  ...
  showhideONmethod: function (cmp, evt, helper) {
    ...
    switch (acMethod) {
      ...
      case "1":
        ...
        $A.util.removeClass(
          cmp.find("sanctionScreeningResultedInNegativeTrace"),
          "slds-hide"
        );
  ...
  calculateCreditCardScore: function (cmp, evt, helper) {
    console.log("Test============================1");
    var Empcategory =
      cmp.find("EmployementType").get("v.value") == "1"
        ? "Salaried"
        : "Self-Employed";
    ...
    var ResidenceYr =
      cmp.find("YearsAtCurrentresidence").get("v.value") == ""
        ? "0"
        : cmp.find("YearsAtCurrentresidence").get("v.value");
    console.log("Test============================2");
    ...
    var PoliticallyExposed = helper.PoliticallyExposedPerson(
      cmp.find("PoliticallyExposedPerson").get("v.value")
    );
    var sanctionTrace = helper.sanctionScreeningTrace(
      cmp.find("sanctionScreeningResultedInNegativeTrace").get("v.value")
    );
    console.log("Test============================3");
    var StatusOfEmployment = helper.EmploymentStatus(
      cmp.find("EmploymentStatus").get("v.value")
    );
    var IndustryofEmployment = helper.IndustryofEmployment(
      cmp.find("IndustryofEmployment").get("v.value")
    );
    var LengthofCurrentEmployment = cmp
      .find("LengthofCurrentEmployment")
      .get("v.value");
    var Professional_Category = helper.ProfessionalCategory(
      cmp.find("ProfessionalCategory").get("v.value")
    );
    console.log("Test============================4");
    var CollateralType = helper.Collateral(
      cmp.find("Collateral").get("v.value")
    );
    var LoanValue = cmp.find("LoantoValue").get("v.value");
    var AbilityService = cmp.find("Abilitytoservice").get("v.value");
    var Repayment_Method = helper.RepaymentMethod(
      cmp.find("RepaymentMethod").get("v.value")
    );
    console.log("Test============================5");
    helper.GenrateScore(
      cmp,
