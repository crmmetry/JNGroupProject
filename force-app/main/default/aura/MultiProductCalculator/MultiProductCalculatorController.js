...
({
    ...
    if (isprod) {
      ...
      action.setCallback(this, function (response) {
        ...
        if (state === "SUCCESS") {
          ...
          for (var k in applicantlst) {
            applicants.push(applicantlst[k].accountId);
      ...
      action.setCallback(this, function (response) {
        ...
        if (state === "SUCCESS") {
          ...
          const shouldShowStatements = helper.canShowStatementOfAffairs(
            notAllowedStatementAffairsScenarios,
            productselection
          );
          ...
          if (
            ...
            typeof productselection === "undefined" ||
            productselection == "0"
          ) {
            cmp.set("v.canShowStatementAffairs", true);
          }
          console.warn(
            "canShowStatementAffairs Next",
            cmp.get("v.canShowStatementAffairs")
          );
          if (
            calc == "1" ||
            calc == "5" ||
  ...
  Requestdetaileventhandler: function (cmp, event, helper) {
    ...
    for (var k in RequestData) {
      ...
      if (RequestData[k].colIndex == "Market") {
        ...
        console.log(
          "amla3========" + cmp.find("Othee_post_moratorium__id").get("v.value")
        );
        if (
          cmp.find("Includeamoratoriumofloanrepayments").get("v.value") == 0 &&
          cmp.find("IndicateType").get("v.value") == 1 &&
  ...
  showhideCoverageType: function (cmp, evt, helper) {
    ...
    if (
      cmp.get("v.applicant1age") &&
      cmp.get("v.applicant2age") &&
      cmp.get("v.applicant3age")
    ) {
      var opts = [
        { value: "0", label: "choose one..." },
  ...
  showhideONLoanPurpose: function (cmp, evt, helper) {
    ...
    if (isprod) {
      ...
      cmp.set("v.lblMinimum", "Minimum (MV,PP)");
    }
    ...
    switch (acMethod) {
      ...
      case "1":
        ...
        $A.util.removeClass(
          cmp.find("autoPurchasePriceOfVehicleCurr"),
          "slds-hide"
        );
    ...
    $A.enqueueAction(cmp.get("c.CalculateMinimumMV"));
    console.log(
      "Minimum value=============>" + cmp.find("Minimummv").get("v.value")
    );
  },
      ...
      case "1":
        ...
        if (isprod) {
          ...
          if (financial == "JN Fund Managers Limited" && accounttype == "Repo")
            SecuredCollateral = "3";
          if (
            financial == "JN Fund Managers Limited" &&
  ...
  CreditCardCalculationSwitch: function (cmp, evt, helper) {
    //JN1-2490
  ...
  locCollateralTypeOnChange: function (cmp, evt, helper) {
    ...
    switch (type) {
      ...
      case "1":
        ...
        $A.util.removeClass(
          cmp.find("CASH_INVESTMENTDETAILS1PLOC"),
          "slds-hide"
        );
        $A.util.addClass(cmp.find("locRealEstateDiv1"), "slds-hide");
        $A.util.addClass(cmp.find("locRealEstateDiv2"), "slds-hide");
  ...
  lineOfCreditCalculation: function (cmp, evt, helper) {
    ...
    console.log(
      "LOC ExistingMonthlyCreditPayment====" + ExistingMonthlyCreditPayment
    );
    ...
    switch (CollateralType) {
      ...
      case "1":
        ...
        if (isprod) {
          var financial = cmp
            .find("Financial_Institution_LOC__id")
            .get("v.value");
  ...
  SaveData: function (cmp, evt, helper) {
    console.log(
      "applicant1age-------------------------1-" + cmp.get("v.applicant1age")
    );
    console.log(
      "applicant2age-------------------------1-" + cmp.get("v.applicant2age")
    );
    console.log(
      "applicant2age-------------------------1-" + cmp.get("v.applicant3age")
    );
    ...
    if (isprod) {
      ...
      if (!isPortionValid) {
        ...
        toastEvent.setParams({
          title: "Error",
          message:
            "Please update the fields against page validation/s to proceed and Save.",
          duration: " 5000",
          key: "info_alt",
          type: "Error",
          mode: "pester"
    ...
    if (numberapplst != undefined) numberapp = numberapplst.length;
    console.log("pavit-1");
    ...
    if (
      ...
      if (numberapp >= 1) {
        applicant1name =
          numberapplst[0].FirstName + " " + numberapplst[0].LastName;
        ...
        if (
          dob1 == false &&
          (coveragetypeselected == 1 ||
            coveragetypeselected == 4 ||
            coveragetypeselected == 5 ||
            coveragetypeselectedUn == 1 ||
            coveragetypeselectedUn == 4 ||
            coveragetypeselectedUn == 5)
        ) {
      ...
      if (numberapp >= 2) {
        applicant2name =
          numberapplst[1].FirstName + " " + numberapplst[1].LastName;
        console.log("pavit2");
        ...
        if (
          dob2 == false &&
          (coveragetypeselected == 2 ||
            coveragetypeselected == 4 ||
            coveragetypeselected == 6 ||
            coveragetypeselectedUn == 2 ||
            coveragetypeselectedUn == 4 ||
            coveragetypeselectedUn == 6)
        ) {
          if (invaliddobstr == "ok")
      ...
      if (numberapp == 3) {
        applicant3name =
          numberapplst[2].FirstName + " " + numberapplst[2].LastName;
    ...
    if (
      ...
      Market_currency_of_vehicle = cmp
        .find("MarketValueofVehicle")
        .get("v.value");
      ...
      if (cmp.find("LoanPurpose").get("v.value") == "2")
        Loan_Balance_of_Vehicle = cmp
          .find("LoanBalanceofVehicle")
          .get("v.value");
      ...
      Monthly_JNGI_Motor_Premium_1st_Year = cmp
      ...
      if (
        ...
        Othee_post_moratorium = cmp
          ...
          .get("v.value");
        Monthly_JN_Life_Creditor_Life_Premium_M = cmp
          .find("MonthlyJNLifeCreditorLifePremium2")
          .get("v.value");
        Monthly_Loan_Payment_after_moratorium = cmp
          .find("MonthlyLoanPayment2")
          .get("v.value");
        Monthly_loan_Payment_Monthly_Savings_M = cmp
      ...
      console.log(
        "pavit--------------------------Auto Loan" + RequestDataAuto.length
      );
      if (RequestDataAuto.length >= 2) {
        console.log("pavit--------------------------Auto Loanstaff1");
        JN_Staff1_intereste_rate = RequestDataAuto[1].Interestrate;
        JN_Staff1_loan_term =
          parseFloat(RequestDataAuto[1].LoanTerm1 * 12) +
          parseFloat(RequestDataAuto[1].LoanTerm2);
        Staff_1_Loan_amount_and_calculations = parseFloat(
          RequestDataAuto[1].LoanAmount
        );
      }
      if (RequestDataAuto.length >= 3) {
        ...
        Staff_2_Loan_amount_and_calculations = parseFloat(
          RequestDataAuto[2].LoanAmount
        );
      }
      if (RequestDataAuto.length >= 4) {
        ...
        Staff_3_Loan_amount_and_calculations = parseFloat(
          RequestDataAuto[3].LoanAmount
        );
      }

      console.log("pavit1--------------------------Auto Loan");
      Total_Monthly = cmp.find("MonthlyLoanPayment1").get("v.value");
      Monthly_Loan_Payment = cmp
        .find("MonthlyLoanPaymentMarket1")
        .get("v.value");
      JN_Life_Creditor_Life_Premium = cmp
        .find("JNLifeCreditorLifePremium1")
        .get("v.value");
      JN_life_Creditor_Life_Premium_moratorium = cmp
        .find("MonthlyJNLifeCreditorLifePremium2")
        .get("v.value");
      Monthly_JNGI_Motor_Premium_moratorium = cmp
        .find("MonthlyJNGIMotorPremium1stYear2")
        .get("v.value");
      Monthly_JNGI_Motor_Premium_1_12_Pay = cmp
        .find("MonthlyJNGIMotorPremiumhalfPayment1")
        .get("v.value");
      Monthly_JN_Life_Creditor_Life_Premium = cmp
        .find("MonthlyJNLifeCreditorLifePremium1")
        .get("v.value");
      Monthly_Processing_Fees_moratorium = cmp
        .find("MonthlyProcessingFees2")
        .get("v.value");
      console.log("pavit2--------------------------Auto Loan");
      if (cmp.find("WaiveProcessingFee").get("v.value") == "1") {
        Processing_Fees_including_GCT = cmp
          .find("ProcessingFeesincludingGCT1")
          .get("v.value");
        Monthly_Processing_Fees_During_moratoriu = cmp
      ...
      JNGI_Motor_Premium_1st_Year = cmp
        .find("JNGIMotorPremium1stYear1")
        .get("v.value");
      if (RequestDataAuto.length >= 2) {
        Staff1_Mthly_Loan_Payment_D_Moratorium = cmp
          .find("MonthlyLoanPaymentJN11")
          .get("v.value");
        Staff1_Mthly_Loan_Payment_A_Moratorium = cmp
          .find("MonthlyLoanPaymentJN12")
          .get("v.value");
      }
      if (RequestDataAuto.length >= 3) {
        Staff2_Mthly_Loan_Payment_D_Moratorium = cmp
          .find("MonthlyLoanPaymentJN21")
          .get("v.value");
        Staff2_Mthly_Loan_Payment_A_Moratorium = cmp
          .find("MonthlyLoanPaymentJN22")
          .get("v.value");
      }
      if (RequestDataAuto.length >= 4) {
        Staff3_Mthly_Loan_Payment_D_Moratorium = cmp
          .find("MonthlyLoanPaymentJN31")
          .get("v.value");
        Staff3_Mthly_Loan_Payment_A_Moratorium = cmp
      ...
      if (isprod) {
        ...
        console.log(
          'cmp.find("ApplicantCount").get("v.value")-----------Auto Loan' +
            cmp.get("v.ApplicantCount")
        );
        if (cmp.get("v.ApplicantCount") >= 1) {
          App1_Loan_Amt_Alloc = cmp.find("LoanAmountauto11").get("v.value");
          App1_Mnthly_Pmt_Alloc = cmp
            .find("MonthlyPaymentauto11")
            .get("v.value");
          App_1_Loan_Amount_A = parseFloat(
            cmp.find("LoanAmountauto1").get("v.value")
          );
          App1_Monthly_Payment_A = parseFloat(
            cmp.find("MonthlyPaymentauto1").get("v.value")
          );
        }
        if (cmp.get("v.ApplicantCount") >= 2) {
          App2_Loan_Amt_Alloc = cmp.find("LoanAmountauto21").get("v.value");
          App2_Mnthly_Pmt_Alloc = cmp
            .find("MonthlyPaymentauto21")
            .get("v.value");
          App_2_Loan_Amount_A = parseFloat(
            cmp.find("LoanAmountauto2").get("v.value")
          );
          App_2_Monthly_Payment_A = parseFloat(
            cmp.find("MonthlyPaymentauto2").get("v.value")
          );
        }
        if (cmp.get("v.ApplicantCount") >= 3) {
          App3_Loan_Amt_Alloc = cmp.find("LoanAmountauto31").get("v.value");
          App3_Mnthly_Pmt_Alloc = cmp
            .find("MonthlyPaymentauto31")
            .get("v.value");
          App_3_Loan_Amount_A = parseFloat(
            cmp.find("LoanAmountauto3").get("v.value")
          );
          App_3_Monthly_Payment_A = parseFloat(
            cmp.find("MonthlyPaymentauto3").get("v.value")
          );
        }
        ...
        if (cmp.find("Deductthe1stmonthrepayment").get("v.value") == "1") {
          ...
          cmp.find("Deductthe1stmonthrepayment").get("v.value") == "2"
        ) {
          Deduct1stmonthAuto = "No";
        }
        Vehicle_Classification = cmp
          .find("Vehicle_Classification__id")
          .get("v.value");
        Desired_Monthly_Repayment_Date = cmp
          .find("Desired_Monthly_Repayment_Date__id")
          .get("v.value");
        Make_and_Model_of_Motor_Vehicle = cmp
          .find("Make_and_Model_of_Motor_Vehicle__id")
          .get("v.value");
    ...
    if (
      ...
      if (cmp.find("InterestedinCreditorLifeUn").get("v.value") == "0") {
        ...
        unsecure_JN_Life_Creditor_Life_Premium = cmp
          .find("JNLifeCreditorLifePremium1Un")
          .get("v.value");
        Unsecure_Monthly_JN_Life_Premium = cmp
      ...
      if (cmp.find("WaiveProcessingFeeUn").get("v.value") == "1") {
        waivefeeun = "No";
        Unsecure_Monthly_Processing_Fees = cmp
      ...
      Unsecure_Monthly_Loan_Payment_Market = cmp
        .find("MonthlyLoanPaymentMarket1Un")
        .get("v.value");
      ...
      unsecure_Monthly_n_Loan_Savings = cmp
      ...
      if (isprod) {
        if (cmp.get("v.ApplicantCount") >= 1) {
          ...
          App1_Mnthly_Pmt_Alloc_UL = cmp
            .find("MonthlyPaymentUn11")
            .get("v.value");
          App_1_Loan_Amount_UL = parseFloat(
            cmp.find("LoanAmountUn1").get("v.value")
          );
          App_1_Monthly_Payment_UL = cmp
            .find("MonthlyPaymentUn1")
            .get("v.value");
        }
        if (cmp.get("v.ApplicantCount") >= 2) {
          App2_Loan_Amt_Alloc_UL = cmp.find("LoanAmountUn21").get("v.value");
          App2_Mnthly_Pmt_Alloc_UL = cmp
            .find("MonthlyPaymentUn21")
            .get("v.value");
          App_2_Loan_Amount_UL = parseFloat(
            cmp.find("LoanAmountUn2").get("v.value")
          );
          App_2_Monthly_Payment_UL = parseFloat(
            cmp.find("MonthlyPaymentUn2").get("v.value")
          );
        }

        if (cmp.get("v.ApplicantCount") >= 3) {
          App3_Loan_Amt_Alloc_UL = cmp.find("LoanAmountUn31").get("v.value");
          App3_Mnthly_Pmt_Alloc_UL = cmp
            .find("MonthlyPaymentUn31")
            .get("v.value");
          App_3_Loan_Amount_UL = parseFloat(
            cmp.find("LoanAmountUn3").get("v.value")
          );
          App_3_Monthly_Payment_UL = parseFloat(
            cmp.find("MonthlyPaymentUn3").get("v.value")
          );
        }
        Repayment_Method_UL = cmp
          .find("Repayment_Method_UL__id")
          .get("v.value");
        Desired_Monthly_Repayment_Date_UL = cmp
          .find("Desired_Monthly_Repayment_Date_UL__id")
          .get("v.value");
    ...
    if (
      ...
      Minimum_Payment_as_per_Credit_Limit = cmp
        .find("ccMinimumPaymentAsPerCreditLimit")
        .get("v.value");
      Annual_Membership = cmp.find("ccAnnualMembership").get("v.value");
      Annual_Membership_Per_sulpplementary =
        "" +
      ...
      PC_Collateral_Type =
        cmp.find("ccCollateralType").get("v.value") == "0"
          ? "None"
          : "Cash/Investment";
      ...
      PC_Account_Hypothecated_for_another_Loan =
        ...
        "0"
          ? "No"
          : "Yes";
      PC_Total_existing_Loan_Balance = cmp
        .find("ccTotalExistingLoanBalance")
        .get("v.value");
      ...
      if (!isprod) {
        ...
        switch (SecuredCollateral) {
          case "1":
            ...
            break;
          case "2":
            PC_Type_of_Cash_Secured_Collateral =
              "JN Bank - Direct Gain Account";
            break;
          case "3":
      ...
      if (isprod) {
        ...
        interest_in_JN_Life_CC_Creditor_Life_iIn = cmp
          .find("CCLifeInsurance")
          .get("v.value");
        ...
        console.log(
          "11/22/2019--------------------------3.3" +
            cmp.find("Account_Number__id").get("v.value")
        );
        Desired_Statement_Date = cmp
          .find("DesiredMonthlyRepaymentDateCC")
          .get("v.value");
    ...
    if (
      ...
      console.log(
        "testk--------------------------1-1" +
          cmp.find("locStartingLimit").get("v.value")
      );
      Line_of_Credit_Starting_Limit = cmp
        .find("locStartingLimit")
        .get("v.value");
      console.log(
        "testk--------------------------1-1" +
      ...
      switch (pclCT) {
        ...
        case "1":
          ...
          if (!isprod) {
            var pclSecuredCollateral = cmp
              .find("locTypeOfCashSecuredCollateral")
              .get("v.value");
            switch (pclSecuredCollateral) {
              case "1":
                PCL_Type_of_Cash_Secured_Collateral =
                  "JN Bank - High Yield Account";
                break;
              case "2":
                PCL_Type_of_Cash_Secured_Collateral =
                  "JN Bank - Direct Gain Account";
        ...
        case "2":
          ...
          console.log(
            "testk--------------------------1-market" +
              cmp.find("locMarketValueofProperty").get("v.value")
          );
          Market_Value_of_Propert = cmp
            .find("locMarketValueofProperty")
            .get("v.value");
          break;
      }
      console.log("testk--------------------------1-");
      if (isprod) {
        if (cmp.get("v.ApplicantCount") >= 1) {
          ...
          App1_Portion_Min_Pay_LOC = cmp
            .find("MinimumPayment11")
            .get("v.value");
          App_1_LOC_limit = parseFloat(cmp.find("LOCLimit1").get("v.value"));
          App_1_Min_Payment = parseFloat(
            cmp.find("MinimumPayment1").get("v.value")
          );
        }
        if (cmp.get("v.ApplicantCount") >= 2) {
          App2_Portion_LOC_Lim = cmp.find("LOCLimit21").get("v.value");
          App2_Portion_Min_Pay_LOC = cmp
            .find("MinimumPayment21")
            .get("v.value");
          App_2_LOC_Limit = parseFloat(cmp.find("LOCLimit2").get("v.value"));
          App_2_Min_Payment_LOC = parseFloat(
            cmp.find("MinimumPayment2").get("v.value")
          );
        }
        if (cmp.get("v.ApplicantCount") >= 3) {
          App3_Portion_LOC_Lim = cmp.find("LOCLimit31").get("v.value");
          App3_Portion_Min_Pay_LOC = cmp
            .find("MinimumPayment31")
            .get("v.value");
          App_3_LOC_Limit = parseFloat(cmp.find("LOCLimit3").get("v.value"));
          App_3_Min_Payment_LOC = parseFloat(
            cmp.find("MinimumPayment3").get("v.value")
          );
        }

        var pclAcc = cmp.find("AccountTypeloc").get("v.value");

        Account_Holders_LOC = cmp
          .find("Account_Holders_LOC__id")
          .get("v.value");
        Desired_Monthly_Repayment_Date_LOC = cmp
          .find("Desired_Monthly_Repayment_Date_LOC__id")
          .get("v.value");
        Annual_Interest_Rate_on_Deposit_Acc_LOC = cmp
          .find("Annual_Interest_Rate_on_Deposit_Acc_LOC__id")
          .get("v.value");
        Financial_Institution_LOC = cmp
          .find("Financial_Institution_LOC__id")
          .get("v.value");
        ...
        Is_Property_a_Strata = cmp
          ...
          .get("v.value");
        Legal_Related_Activities = cmp
          .find("Legal_Related_Activities__id")
          .get("v.value");
        Name_of_Valuer = cmp.find("Name_of_Valuer__id").get("v.value");
        Occupancy_Type = cmp.find("Occupancy_Type__id").get("v.value");
        Property_Classification = cmp
          .find("Property_Classification__id")
          .get("v.value");
        Property_Title_Information_Folio = cmp
          .find("Property_Title_Information_Folio__id")
          .get("v.value");
        Property_Title_information_Vol = cmp
          .find("Property_Title_information_Vol__id")
          .get("v.value");
    ...
    var newCalculator = Object.assign(
      newCalculator1,
      newCalculator2,
      newCalculator3,
      newCalculator4,
    ...
    if (isprod) {
      ...
      console.log(
        "applicant1age--------------------------" +
          cmp.find("CoverageType").get("v.value")
      );
      console.log(
        "applicant2age--------------------------" + cmp.get("v.applicant2age")
      );
      console.log(
        "applicant2age--------------------------" + cmp.get("v.applicant3age")
      );
    }
  },
  //======Product Detail=============
  ...
  handleLookupSelectEventProductDetail: function (cmp, event, helper) {
    ...
    console.log(
      "Record id Promotion Unsecured" + cmp.get("v.UnsecuredPromotion")
    );
    console.log("Record id Promotion Credit" + cmp.get("v.CCPromotion"));
    console.log(
      "Record id Promotion Line of Credit" + cmp.get("v.LOCPromotion")
    );
  },
  ApplicationFormDoc: function (cmp, evt, helper) {
    ...
    window.open(
      "" + window.location.origin + "/apex/ApplicationForm" + "?oppid=" + oppid
    );
  },
  ...
  Wasexceptiongrantonchange: function (cmp, evt, helper) {
    ...
    switch (type) {
      case "1":
        $A.util.removeClass(
          cmp.find("ExceptionalTermsApprovedBy"),
          "slds-hide"
        );
  ...
  WasexceptiongrantonchangeUn: function (cmp, evt, helper) {
    ...
    switch (type) {
      case "1":
        $A.util.removeClass(
          cmp.find("ExceptionalTermsApprovedByUn"),
          "slds-hide"
        );
  ...
  WasexceptiongrantonchangeCC: function (cmp, evt, helper) {
    ...
    switch (type) {
      case "1":
        $A.util.removeClass(
          cmp.find("ExceptionalTermsApprovedByCC"),
          "slds-hide"
        );
  ...
  WasexceptiongrantonchangeLOC: function (cmp, evt, helper) {
    ...
    switch (type) {
      case "1":
        $A.util.removeClass(
          cmp.find("ExceptionalTermsApprovedByLOC"),
          "slds-hide"
        );
  ...
  CalculateMinimumMV: function (cmp, evt, helper) {
    ...
    if (isprod) {
      ...
      switch (acMethod) {
        case "0":
        case "1":
          PurchasePriceofVehicle = cmp
            .find("PurchasePriceofVehicle")
            .get("v.value");
          MarketValueofVehicle = cmp
            .find("MarketValueofVehicle1")
            .get("v.value");
      ...
      if (PurchasePriceofVehicle == "") PurchasePriceofVehicle = 0;
      if (MarketValueofVehicle == "") MarketValueofVehicle = 0;
      if (PurchasePriceofVehicle == undefined) PurchasePriceofVehicle = 0;
      if (MarketValueofVehicle == undefined) MarketValueofVehicle = 0;
      ...
      if (PurchasePriceofVehicle == "" && MarketValueofVehicle == "") {
        ...
        cmp
          ...
          .set(
            "v.value",
            Math.min(PurchasePriceofVehicle, MarketValueofVehicle)
          );
      }
      console.log(
        "Minimum value=============1>" + cmp.find("Minimummv").get("v.value")
      );
    }
  },
  ...
  calculateApplicantUnSecurePortion: function (cmp) {
    ...
    if (count >= 1) {
      ...
      if (loanamt1 != "") totalLoan += parseFloat(loanamt1);
    }
    if (count >= 2) {
      loanamt2 = cmp.find("LoanAmountUn2").get("v.value");
      if (loanamt2 != "") totalLoan += parseFloat(loanamt2);
    }
    if (count >= 3) {
      loanamt3 = cmp.find("LoanAmountUn3").get("v.value");
      if (loanamt3 != "") totalLoan += parseFloat(loanamt3);
    }
    ...
    console.log(
      "isUnsecureLoanPortionValid========>" +
        cmp.get("v.isUnsecureLoanPortionValid")
    );
  },
  calculateApplicantUnSecurePortionPay: function (cmp) {
    ...
    if (count >= 1) {
      ...
      if (monthlyPay1 != "") totalPay += parseFloat(monthlyPay1);
    }
    if (count >= 2) {
      monthlyPay2 = cmp.find("MonthlyPaymentUn2").get("v.value");
      if (monthlyPay2 != "") totalPay += parseFloat(monthlyPay2);
    }
    if (count >= 3) {
      monthlyPay3 = cmp.find("MonthlyPaymentUn3").get("v.value");
      if (monthlyPay3 != "") totalPay += parseFloat(monthlyPay3);
    }
    ...
    console.log(
      "isUnsecurePayPortionValid========>" +
        cmp.get("v.isUnsecurePayPortionValid")
    );
  },
  ...
  calculateApplicantAutoPortion: function (cmp) {
    ...
    console.log(
      "isAutoLoanPortionValid========>" + cmp.get("v.isAutoLoanPortionValid")
    );
  },
  calculateApplicantAutoPortionPay: function (cmp) {
    ...
    console.log(
      "isAutoPayPortionValid========>" + cmp.get("v.isAutoPayPortionValid")
    );
  },
  ...
  calculateApplicantLocPortion: function (cmp) {
    ...
    console.log(
      "isLocLoanPortionValid========>" + cmp.get("v.isLocLoanPortionValid")
    );
  },
  calculateApplicantLocPortionPay: function (cmp) {
    ...
    console.log(
      "isLocPayPortionValid========>" + cmp.get("v.isLocPayPortionValid")
    );
  },
  GenerateMultiCalcPdf: function (cmp, event, helper) {
    ...
    window.open(
      "" +
        window.location.origin +
        "/apex/MultiProductCalculatorDoc?loanid=" +
        loanid +
        "&location=" +
        cmp.get("v.selectedLocation")
    );
  },
  creditScoring: function (cmp, event, helper) {
    ...
    if (id_str == "Auto") {
      ...
      window.open(
        "" + window.location.origin + "/apex/CreditScoringModel" + str
      );
    }
