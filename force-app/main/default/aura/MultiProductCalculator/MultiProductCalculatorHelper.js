({	
    ...
    setAllApplicants: function(cmp){
        const allApplicants = cmp.get("v.allApplicants");
        const accountId = cmp.get("v.accountId");
        let applicants = {};
        let count = 1;
        allApplicants.forEach(function(applicant){
            if(this.isMainApplicant(applicant.Account__c, accountId)) {
    ...
    sumAllWith: function( obj, props){
        let sum = 0;
        props.forEach(prop => {
            sum += this.cv(obj[prop]);
        });
        return sum;
    },
    calculateExistingMonthlyPaymentBefore : function(cmp, value, type){
        let existingMonthlyPaymentBefore = cmp.get("v.existingMonthlyPaymentBefore");
        if(existingMonthlyPaymentBefore[type]){
            existingMonthlyPaymentBefore[type] += this.cv(value);
        } else {
            existingMonthlyPaymentBefore[type] = 0;
        }
       
        cmp.set("v.existingMonthlyPaymentBefore", existingMonthlyPaymentBefore);
    },
    calculateExistingMonthlyPaymentAfter : function(cmp, value, type){
        let existingMonthlyPaymentAfter = cmp.get("v.existingMonthlyPaymentAfter");
        if(existingMonthlyPaymentBefore[type]){
            existingMonthlyPaymentBefore[type] += this.cv(value);
        } else {
            existingMonthlyPaymentBefore[type] = 0;
        }
        cmp.set("v.existingMonthlyPaymentAfter", existingMonthlyPaymentAfter);
    },
    calculateNewInstallmentPaymentsBasic: function (cmp) {
        const l = cmp.get("v.loanCalculation");
        return this.sumAll([l.Monthly_Loan_Payment__c, l.JNGI_Motor_Premium_1st_Year__c,l.JN_Life_Creditor_Life_Premium__c,l.Monthly_Processing_Fees__c])
    },

    calculateTDSRValues : function(cmp, applicationAssetLiability){
         const selection = cmp.find("selectapplicant").get("v.value");
            ...
            case "1":{
                //Auto Loan
                this.calculateAutoLoanBefore(cmp, applicationAssetLiability);
                this.calculateAutoLoanAfter(cmp, applicationAssetLiability);
                break;
            }
            case "2":{
                //Unsecured Loan
    ...
    getApplicantLoanAssets: function(cmp){
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS"){
                const applicantAssets = response.getReturnValue()[0];
                const allApplicationAssetLiabilities = applicantAssets.ApplicationAssetLiability;
                const loanCalculation = applicantAssets.loanCalculation;
                const allApplicants = applicantAssets.Applicantlst;
                const accountId = applicantAssets.accountId;
                cmp.set("v.allApplicants", allApplicants);
                cmp.set("v.loanCalculation", loanCalculation);
                cmp.set("v.allApplicationAssetLiabilities", allApplicationAssetLiabilities);
    ...
    calculateTDSRBeforeandAfter: function(cmp){
        const allApplicants = cmp.get("v.allApplicants");
        const loanCalculation = cmp.get("v.loanCalculation");
        const allApplicationAssetLiabilities = cmp.get("v.allApplicationAssetLiabilities");
        const newInstallmentPayments = {
            unsecured_loan: 0,
            credit_card: 0,
            line_credit: 0,
        };    
        const prevInstallmentPayments = {
            auto_unsecured: 0,
            auto_unsecured_after: 0,
            auto_credit: 0,
            auto_credit_after:0,
            auto_line:0,
            auto_line_after:0,
            auto_credit_line:0,
            auto_credit_line_after:0,
            auto_unsecured_line:0,
            auto_unsecured_line_after:0,
            auto_unsecured_credit:0,
            auto_unsecured_credit_after:0,  
            unsecured_credit_line: 0,
            unsecured_credit_line_after: 0,
            auto_unsecured_credit_line: 0,
            auto_unsecured_credit_line_after: 0,
            same_before: 0,
            same_after: 0
        }; 
        for(let index of allApplicationAssetLiabilities){
            let applicationAssetLiability = allApplicationAssetLiabilities[index];
            //new new Installment Payments
            newInstallmentPayments['unsecured_loan'] += this.cv(this.loanCalculation.Unsecure_Monthly_Loan_Payment__c) + 
            this.cv(applicationAssetLiability.Assets_and_Liabilities__r.Percent_Applicant_Portion__c);

            newInstallmentPayments['credit_card'] += this.cv(loanCalculation.Minimum_Payment_as_per_Credit_Limit__c);

            newInstallmentPayments['line_credit'] += this.cv(loanCalculation.Minimum_Payment_as_per_Credit_Limit__c);

            prevInstallmentPayments['same_before'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c' ]);

            prevInstallmentPayments['auto_unsecured'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c' ]);

            prevInstallmentPayments['auto_unsecured_after'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 'JNGI_Motor_Premium_1st_Year__c', 
                'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c', 'Unsecure_Monthly_Loan_Payment__c']);
        
            prevInstallmentPayments['auto_credit'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c' ]);

            prevInstallmentPayments['auto_credit_after'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c' ,'Minimum_Payment__c']);

            prevInstallmentPayments['auto_line_after'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c','Minimum_Payment_as_per_Credit_Limit__c' ]);

            prevInstallmentPayments['auto_credit_line'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c','Minimum_Payment__c' ]);

            prevInstallmentPayments['auto_credit_line_after'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c','Minimum_Payment_as_per_Credit_Limit__c','Minimum_Payment__c' ]);

            prevInstallmentPayments['auto_unsecured_line'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c','Unsecure_Monthly_Loan_Payment__c' ]);

            prevInstallmentPayments['auto_unsecured_line_after'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c','Unsecure_Monthly_Loan_Payment__c',
            'Minimum_Payment_as_per_Credit_Limit__c' ]);

            prevInstallmentPayments['auto_unsecured_credit'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c','Unsecure_Monthly_Loan_Payment__c' ]);

            prevInstallmentPayments['auto_unsecured_credit_after'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c','Unsecure_Monthly_Loan_Payment__c',
            'Minimum_Payment__c' ]);

            prevInstallmentPayments['unsecured_credit_line'] += this.sumAllWith([loanCalculation, 'Unsecure_Monthly_Loan_Payment__c', 
            'Minimum_Payment__c']);

            prevInstallmentPayments['unsecured_credit_line_after'] += this.sumAllWith([loanCalculation, 'Unsecure_Monthly_Loan_Payment__c', 
            'Minimum_Payment__c','Minimum_Payment_as_per_Credit_Limit__c']);

            prevInstallmentPayments['auto_unsecured_credit_line'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c','Unsecure_Monthly_Loan_Payment__c' ]);

            prevInstallmentPayments['auto_unsecured_credit_line_after'] += this.sumAllWith([loanCalculation, 'Monthly_Loan_Payment__c', 
            'JNGI_Motor_Premium_1st_Year__c', 'JN_Life_Creditor_Life_Premium__c', 'Monthly_Processing_Fees__c','Unsecure_Monthly_Loan_Payment__c' ,
            'Minimum_Payment_as_per_Credit_Limit__c']);
            ...
            switch(applicationAssetLiability.Assets_and_Liabilities__r.DeveloperName){                    
                case "Real_Estate":{
                    //auto loans
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Monthly_Payment_Prior__c,'auto_loan' );
                    this.calculateExistingMonthlyPaymentAfter(cmp, applicationAssetLiability.Monthly_Payment_After__c ,'auto_loan');
                    //unsecured loans
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Monthly_Payment_Prior__c,'unsecured_loan' );
                    this.calculateExistingMonthlyPaymentAfter(cmp, applicationAssetLiability.Monthly_Payment_After__c ,'unsecured_loan');
                    //credit cards
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Monthly_Payment_Prior__c,'credit_card' );
                    this.calculateExistingMonthlyPaymentAfter(cmp, applicationAssetLiability.Monthly_Payment_After__c ,'credit_card');
                    //line of credit
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Monthly_Payment_Prior__c,'line_credit' );
                    this.calculateExistingMonthlyPaymentAfter(cmp, applicationAssetLiability.Monthly_Payment_After__c ,'line_credit');
                    //same
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Monthly_Payment_Prior__c,'same' );
                    this.calculateExistingMonthlyPaymentAfter(cmp, applicationAssetLiability.Monthly_Payment_After__c ,'same');
                    break;
                }
                case "Motor_Vehicle":{
                    //auto loans
                ...
                case "Credit_Cards": {
                     //auto loans
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Minimum_Payment__c,'auto_loan' );
                    this.calculateExistingMonthlyPaymentAfter(cmp, applicationAssetLiability.Minimum_Payment_After__c,'auto_loan' );
                    //unsecured loans
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Minimum_Payment__c,'unsecured_loan' );
                    this.calculateExistingMonthlyPaymentAfter(cmp, applicationAssetLiability.Minimum_Payment_After__c ,'unsecured_loan');
                    //credit cards
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Minimum_Payment__c,'credit_card' );
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Minimum_Payment_After__c,'credit_card' );
                    //line of credit
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Minimum_Payment__c,'line_credit' );
                    this.calculateExistingMonthlyPaymentAfter(cmp, applicationAssetLiability.Minimum_Payment_After__c ,'line_credit');
                    //same
                    this.calculateExistingMonthlyPaymentBefore(cmp, applicationAssetLiability.Minimum_Payment__c,'same' );
                    this.calculateExistingMonthlyPaymentAfter(cmp, applicationAssetLiability.Minimum_Payment_After__c ,'same');
        ...
        for(let index of allApplicants){
            let applicant = allApplicants[index];
            //for auto loan before
            grossMonthlyIncomeCTotal += this.cv(applicant.Gross_monthly_incomeC_c);            
        }
        //auto loan before and after final calculation
        const autoLoanBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['auto_loan'] / grossMonthlyIncomeCTotal);
        const autoLoanAfter = parseFloat(cmp.get("v.existingMonthlyPaymentAfter")['auto_loan'] + this.calculateNewInstallmentPaymentsBasic())/grossMonthlyIncomeCTotal;

        //unsecured loan before and after final calculation
        const unsecuredLoanBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['unsecured_loan']/grossMonthlyIncomeCTotal);
        const unsecuredLoanAfter = parseFloat(cmp.get("v.existingMonthlyPaymentAfter")['unsecured_loan']+newInstallmentPayments['unsecured_loan']/grossMonthlyIncomeCTotal);

        //credit card before and after final calculation 
        const creditCardBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['credit_card'] / grossMonthlyIncomeCTotal);
        const creditCardAfter = parseFloat(cmp.get("v.existingMonthlyPaymentAfter")['credit_card']+newInstallmentPayments['credit_card']/grossMonthlyIncomeCTotal);

        //line of credit before and after final calculation 
        const lineCreditBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['line_credit'] / grossMonthlyIncomeCTotal);
        const lineCreditAfter = parseFloat(cmp.get("v.existingMonthlyPaymentAfter")['line_credit']+newInstallmentPayments['line_credit']/grossMonthlyIncomeCTotal);

        //auto loan and unsecured before and after final calculation 
        const autoUnsecuredBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_unsecured'] / grossMonthlyIncomeCTotal);
        const autoUnsecuredAfter = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_unsecured_after'] / grossMonthlyIncomeCTotal);

        //auto loan and credit card before and after final calculation 
        const autoCreditBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_credit'] / grossMonthlyIncomeCTotal);
        const autoCreditAfter  = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_credit_after'] / grossMonthlyIncomeCTotal);

        //auto loan and line of credit before and after final calculation 
        const autoLineBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['same_before'] / grossMonthlyIncomeCTotal);
        const autoLineAfter  = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_line_after'] / grossMonthlyIncomeCTotal);
               
        //auto ,credit and line of credit before and after final calculation 
        const autoCreditLineBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_credit_line'] / grossMonthlyIncomeCTotal);
        const autoCreditLineAfter  = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_credit_line_after'] / grossMonthlyIncomeCTotal);
                
        //auto ,unsecured and line of credit before and after final calculation 
        const autoUnsecuredLineBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_unsecured_line'] / grossMonthlyIncomeCTotal);
        const autoUnsecuredLineAfter  = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_unsecured_line_after'] / grossMonthlyIncomeCTotal);

         //auto ,unsecured and credit of credit before and after final calculation 
         const autoUnsecuredCreditBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_unsecured_credit'] / grossMonthlyIncomeCTotal);
         const autoUnsecuredCreditAfter  = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_unsecured_credit_after'] / grossMonthlyIncomeCTotal);
         
        //unsecured , credit lineand credit of credit before and after final calculation 
        const unsecuredCreditLineBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['unsecured_credit_line'] / grossMonthlyIncomeCTotal);
        const unsecuredCreditLineAfter  = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['unsecured_credit_line_after'] / grossMonthlyIncomeCTotal);

        //auto, unsecured , credit and line of credit before and after final calculation 
        const autoUnsecuredCreditLineBefore = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_unsecured_credit_line'] / grossMonthlyIncomeCTotal);
        const autoUnsecuredCreditLineAfter  = parseFloat(cmp.get("v.existingMonthlyPaymentBefore")['same'] + prevInstallmentPayments['auto_unsecured_credit_line'] / grossMonthlyIncomeCTotal);
    },
    calculateInstallmentInvestments: function(cmp){
        let existingInstallmentPaymentBefore=0;
        let existingInstallmentPaymentAfter=0;

        const loanCalculation = cmp.get("v.loanCalculation");
        const allApplicationAssetLiabilities = cmp.get("v.allApplicationAssetLiabilities");
        const applicants = this.setAllApplicants(cmp);
        //lets exclude specific applicant fields for now
        for(let index of allApplicationAssetLiabilities){
            let applicationAssetLiability = allApplicationAssetLiabilities[index];
            switch(applicationAssetLiability.Assets_and_Liabilities__r.DeveloperName){                    
                case "Real_Estate":{
                    existingInstallmentPaymentBefore += this.cv(applicationAssetLiability.Monthly_Payment_Prior__c) 
                    * this.cv(applicationAssetLiability.Assets_and_Liabilities__r.Percent_Applicant_Portion__c);

                    existingInstallmentPaymentAfter+= this.cv(applicationAssetLiability.Monthly_Payment__c) 
                    * this.cv(applicationAssetLiability.Assets_and_Liabilities__r.Percent_Applicant_Portion__c);
                    break;
                }
                case "Motor_Vehicle":{
                    ...
                    * this.cv(applicationAssetLiability.Assets_and_Liabilities__r.Percent_Applicant_Portion__c);
                    ...
                    existingInstallmentPaymentAfter+= this.cv(applicationAssetLiability.Monthly_Payment__c) 
                    * this.cv(applicationAssetLiability.Assets_and_Liabilities__r.Percent_Applicant_Portion__c)
                    + this.cv(loanCalculation.Monthly_Loan_Payment__c);
                    break;
                }   
                case "Other_Assets":{
                    existingInstallmentPaymentBefore += this.cv(applicationAssetLiability.Monthly_Payment_Prior__c) 
                ...
                case "Other_Loans":{
                    ...
                    * this.cv(applicationAssetLiability.Assets_and_Liabilities__r.Percent_Applicant_Portion__c);
                    ...
                    existingInstallmentPaymentAfter+= this.cv(applicationAssetLiability.Monthly_Payment__c) 
                    * this.cv(applicationAssetLiability.Assets_and_Liabilities__r.Percent_Applicant_Portion__c)
                    + this.cv(loanCalculation.Unsecure_Monthly_Loan_Payment__c);
                    break;
                }

                case "Credit_Cards": {
                    existingInstallmentPaymentBefore += this.cv(applicationAssetLiability.Minimum_payment__c) 
                    * this.cv(applicationAssetLiability.Assets_and_Liabilities__r.Percent_Applicant_Portion__c);

                    existingInstallmentPaymentAfter+= this.cv(applicationAssetLiability.Minimum_Payment_After__c) 
                    * this.cv(applicationAssetLiability.Assets_and_Liabilities__r.Percent_Applicant_Portion__c)
                    + this.cv(loanCalculation.Minimum_Payment__c);
                    break;
                }
                case "Lines_of_Credit":{
                    existingInstallmentPaymentBefore += this.cv(applicationAssetLiability.Minimum_payment__c) 
        ...
        cmp.set("v.existingInstallmentPaymentBefore",existingInstallmentPaymentBefore);
        cmp.set("v.existingInstallmentPaymentAfter",existingInstallmentPaymentAfter);
    },
    //---------------------------------------------
