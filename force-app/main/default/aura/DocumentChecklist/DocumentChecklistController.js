({
  ...
  OnChangeCollateralTypeLOC: function (cmp, evt, helper) {
    ...
    } else $A.util.addClass(cmp.find("Ispropertystrata1"), "slds-hide");
  },
  ...
  documentChecklistDoc: function (cmp, event, helper) {
    ...
    if (validForm == false) {
      ...
      if (id_str == "Applicant") {
        var url =
          "?incr=" +
          cmp.get("v.checkboxGroupValueapplicant") +
          "&kycdone=" +
          cmp.find("MembershipStatusApplicant").get("v.value") +
          "&kyc=" +
          cmp.get("v.checkboxGroupValuekyc") +
          "&autolp=" +
          cmp.find("LoanPurpose").get("v.value") +
          "&autovc=" +
          cmp.find("VehicleClassification").get("v.value") +
          "&autoin=" +
          cmp.find("InterestedinJNGIProgramme").get("v.value") +
          "&uns=" +
          cmp.get("v.checkboxGroupValue") +
          "&credit=" +
          cmp.find("CollateralTypeCredit").get("v.value") +
          "&loc=" +
          cmp.find("CollateralTypeLineofCredit").get("v.value") +
          "&isloc=" +
          cmp.find("Ispropertystrata").get("v.value") +
          "&payslip=" +
          cmp.find("Doesreceivepayslips").get("v.value") +
          "&commision=" +
          cmp.get("v.checkboxGroupValuepayslip") +
          "&location=" +
          cmp.get("v.selectedValue") +
          "&isNone=" +
          isNone;
        window.open(
          "" +
            window.location.origin +
            "/apex/DocumentCriteriaApplicant" +
            url +
            "" +
            "&CalType=" +
            cmp.find("Selectcombination").get("v.value")
        );
      } else {
        var url =
          "?incr=" +
