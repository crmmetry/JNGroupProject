({
  ...
  saveRecords: function (component, event, helper) {
    ...
    let objunsecuredLoanRecords = JSON.parse(
      JSON.stringify(unsecuredLoanRecords)
    );
    ...
    let objmotorvehicleRecords = JSON.parse(
      JSON.stringify(motorvehicleRecords)
    );
    ...
    let objhirePurchaseRecords = JSON.parse(
      JSON.stringify(hirePurchaseRecords)
    );
    ...
    for (let otherElement of objotherRecords) {
      ...
      if (
        otherElement["Id"] != undefined &&
        otherElement["Id"] != null &&
        otherElement["Id"] != ""
      ) {
    ...
    action.setCallback(this, function (response) {
      ...
      if (state === "SUCCESS") {
        ...
        for (let allRecords of controllerResponse) {
          let newRow = {
            DebtInstitution: allRecords.Institution_Debt_List__c,
            DebtAmount: allRecords.Debt_Amount_Number__c,
            Id: allRecords.Id
          };
          let debtTypeSelected = allRecords.Debt_Type_List__c;
          let totalConsolidatedAmount =
            allRecords.Application__r.Total_Amount_Consolidated__c;
          let totalMonthlyAmount =
            allRecords.Application__r.Total_Monthly_Payment__c;
          let existingRows;
          if (debtTypeSelected == "Unsecured Loan") {
            existingRows = component.get("v.unsecuredLoanRecords");
            existingRows.push(newRow);
            ...
            totalDebtMap[length + "_Unsecured Loan"] =
              allRecords.Debt_Amount_Number__c;
          } else if (debtTypeSelected == "Motor Vehicle Loan") {
            existingRows = component.get("v.motorvehicalRecords");
            existingRows.push(newRow);
            component.set("v.motorvehicalRecords", existingRows);
            ...
            let length = existingRows.length - 1;
            totalDebtMap[length + "_Motor Vehicle Loan"] =
              allRecords.Debt_Amount_Number__c;
          } else if (debtTypeSelected == "Mortgage/ Home Equity Loan") {
            existingRows = component.get("v.homeLoanRecords");
            existingRows.push(newRow);
            ...
            let length = existingRows.length - 1;
            totalDebtMap[length + "_Mortgage/ Home Equity Loan"] =
              allRecords.Debt_Amount_Number__c;
          } else if (debtTypeSelected == "Credit Card") {
            existingRows = component.get("v.creditCardRecords");
            existingRows.push(newRow);
            ...
            let length = existingRows.length - 1;
            totalDebtMap[length + "_Credit Card"] =
              allRecords.Debt_Amount_Number__c;
          } else if (debtTypeSelected == "Student Loan") {
            existingRows = component.get("v.studentLoanRecords");
            existingRows.push(newRow);
            ...
            let length = existingRows.length - 1;
            totalDebtMap[length + "_Student Loan"] =
              allRecords.Debt_Amount_Number__c;
          } else if (debtTypeSelected == "Hire Purchase") {
            existingRows = component.get("v.hirePurchaseRecords");
            existingRows.push(newRow);
            ...
            let length = existingRows.length - 1;
            totalDebtMap[length + "_Hire Purchase"] =
              allRecords.Debt_Amount_Number__c;
          } else if (debtTypeSelected == "Other") {
            existingRows = component.get("v.otherRecords");
            existingRows.push(newRow);
